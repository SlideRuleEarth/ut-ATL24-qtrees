...
char const *Matrix_ArrayInterface(Matrix self) {
    char const tmp[] = "{\"data\": [%lu, true], \"shape\": [%lu, %lu], "
        "\"typestr\": \"<f4\", \"version\": 3}";
    memset(self->_array_intrerface, '\0', sizeof(self->_array_intrerface));
    sprintf(self->_array_intrerface, tmp, (size_t)self->data, self->shape[0],
            self->shape[1]);
...
int main() {
    ...
    const char *model_name = "/tmp/test_xgb2_model.json";
    safe_xgboost(XGBoosterSaveModel(booster, model_name));
